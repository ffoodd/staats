...
module.exports = (url) => {
  ...
  css.validate(url, function(error, data) {
    ...
    if(error) {
      ...
      for (let result in results) {
        let test = new Object(results[result]);
          test.status = results[result].errorType;
          test.name   = results[result].message;
  ...
  stats.parse()
    .then(data => {
      ...
      for (let result in results) {
        ...
        let test = new Object(result);
          // 100 is arbitrary value, need a way to define a per result budget!
  ...
  request(url)
    .then(data => {
      ...
      for (let result in results) {
        ...
        if ('tagCounts' != result) {
          // 10 is arbitrary value, need a way to define a per result budget!
          let test = new Object(result);
            test.status = (parseInt(results[result]) < 10);
            test.name   = result;
            test.value  = results[result];
  ...
  a11y(url, function (error, data) {
    ...
    for (let result in results) {
      let test = new Object(results[result]);
        test.status = results[result].result;
